...
from ch._utils import _istensorable, _min_size
from ch._torch import totensor
...
class Transition(object):
    ...
    Represents a (s, a, r, s', term, trunc) tuple.
    ...
    def __init__(self,
        ...
        * `terminated` (tensor) - Is `next_state` a terminal (absorbing) state ?
        * `truncated` (tensor) - Is `num_max_step` reach ?
        * `infos` (dict, *optional*, default=None) - Additional information on
        ...
        values = [state, action, reward, next_state, terminated, truncated, device]
    ...
    def __setstate__(self, state):
        self._fields = ['state', 'action', 'reward', 'next_state', 'terminated', 'truncated']
...
class ExperienceReplay(list):
    ...
    def append(self,
        ...
        * `terminated` (tensor/bool) - Is `next_state` a terminal (absorbing) state ?
        * `truncated` (tensor/bool) - Have `max_num_step` been reached ?
        * infos` (dict, *optional*, default=None) - Additional information on the transition.

        ## Example

        ~~~python
        replay.append(state, action, reward, next_state, terminated, truncated, info={
            'density': density,
        ...
        next_state = totensor(next_state)
        terminated = totensor(terminated)
        truncated = totensor(truncated)
        if self.vectorized:
            num_envs = state.shape[0]
            action = action.reshape(num_envs, -1)
            reward = reward.reshape(num_envs, -1)
            terminated = terminated.reshape(num_envs, -1)
            truncated = truncated.reshape(num_envs, -1)
        sars = Transition(
            state=state,
            action=action,
            reward=reward,
        ...
        * `ExperienceReplay` - New ExperienceReplay containing the sampled transitions.
        ...
        if episodes:
            ...
            if size > 1 and not contiguous:
                ...
                terminateds = self.terminated()
                truncateds = self.truncated()
        ...
        if nsteps == 1:
            ...
            for idx, index in enumerate(indices):
                ...
                terminated = sars.terminated
                truncated = sars.truncated
                reward = sars.reward
                for i in enum_steps:
                    j = i + index
                    if terminated or truncated or j >= replay_size:
                        break
    ...
    def flatten(self):
        ...
        if not self.vectorized:
            return self
        flat_replay = ExperienceReplay(device=self.device, vectorized=False)
        storage = []
        for sars in self._storage:
            vec_num = sars.terminated.shape[0]
            ...
            for field in sars._fields:
                ...
                for trans, val in zip(transitions, values):
                    trans[field] = val
            transitions = [Transition(**trans) for trans in transitions]
            storage += transitions
        return ExperienceReplay(
            storage=storage,
            device=self.device,
